...
def compute_pcs_from_sst(sst_path, n_pcs=10, window=1, step=1):
    """
    Read SST, compute EOF PCs with higher components.
    """
...
def train():
    ...
    n_pcs = 10
    train_set, test_set = prepare_data(n_pcs=n_pcs)
    ...
    model = UNetLiteFiLM(
        ...
        index_dim=index_dim,
        base_filters=16,
        dropout=0.2  # mild dropout
    ).to(device)
    
    # Mild regularization and lr=5e-4
    lr = 5e-4
    ...
    log_dir = os.path.join(config.modelconfig["log_path"] + "_v2_pcs10")
    ...
    writer.close()
    print(f"\nTraining finished! Best Test ACC for V2 with 10 PCs: {best_test_acc:.3f}")
