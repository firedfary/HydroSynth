...
def acc_hybrid_loss(pred, target, mask, mse_weight=0.05):
    ...
    mask: True/1 for valid land pixels, False/0 for ocean/NaN pixels.
    """
    m = mask.float()
...
def prepare_data():
    ...
    cond_file = os.path.join(data_dir, 'lr_data_v3_aligned.npy')
    target_file = os.path.join(data_dir, 'hr_data_v3_aligned.npy')
    
    if not os.path.exists(cond_file) or not os.path.exists(target_file):
        raise FileNotFoundError(f"Aligned datasets not found in {data_dir}. Run prepare_data_v3.py first.")
...
def train():
    ...
    log_dir = os.path.join(config.modelconfig["log_path"] + "_v3_hybrid")
    ...
    for e in range(config.modelconfig["epoch"]):
        ...
        for x_0, cond, mask, pcs in train_loader:
            ...
            # Clean target NaNs for safe hybrid loss calculation
        ...
        if avg_test_acc > best_test_acc:
            ...
            save_path = os.path.join(config.modelconfig["save_weight_path"], "best_model_v3.pt")
            torch.save(model.state_dict(), save_path)
            
            # Also save as best_model_v2.pt if it beats target
    ...
    writer.close()
    print(f"\nTraining finished! Best Test ACC for V3 Hybrid: {best_test_acc:.3f}")
