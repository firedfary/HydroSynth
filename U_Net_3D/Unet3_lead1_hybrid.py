...
def prepare_data():
    """
    Load target, condition (10-channel), and SST PCs. Align and return TensorDatasets.
    """
    # Load aligned Lead-1 data
...
def train():
    ...
    input_channels = train_set[0][1].shape[0]  # condition channels (10)
    ...
    # Use 5e-4 learning rate for hybrid ACC loss
    lr = 5e-4
    ...
    # Custom log directory for lead-1 hybrid
    log_dir = os.path.join(config.modelconfig["log_path"] + "_lead1_hybrid")
    ...
    for e in range(config.modelconfig["epoch"]):
        ...
        if avg_test_acc > best_test_acc:
            ...
            save_path = os.path.join(config.modelconfig["save_weight_path"], "best_model_lead1_hybrid.pt")
    ...
    writer.close()
    print(f"\nTraining finished! Best Test ACC for Lead-1 Hybrid: {best_test_acc:.3f}")
