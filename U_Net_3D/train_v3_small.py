...
def prepare_data():
    ...
    if not os.path.exists(cond_file) or not os.path.exists(target_file):
        raise FileNotFoundError(f"Aligned datasets not found in {data_dir}. Run prepare_data_v4.py first.")
        
    cond = np.load(cond_file).astype(np.float32)      # [N, 11, 120, 140]
    ...
    for target_date in aligned_target_dates:
        ...
        target_months_list.append(target_date.month)
        
    pcs_aligned = np.stack(pcs_aligned_list)
    target_months = np.array(target_months_list)
    
    # Cyclic encoding of target calendar months
    months_sin = np.sin(2 * np.pi * target_months / 12.0)
    months_cos = np.cos(2 * np.pi * target_months / 12.0)
    
    # Concatenate PCs with seasonality features
    pcs_and_months = np.concatenate([pcs_aligned, months_sin[:, None], months_cos[:, None]], axis=1) # [N, 7]
    pcs_t = torch.from_numpy(pcs_and_months).float()
    
    print(f"Data ready. Total samples: {target_t.shape[0]}")
    print(f"Condition shape: {cond_t.shape}")
    print(f"Target shape: {target_t.shape}")
    print(f"PCs + Seasonality shape: {pcs_t.shape}")
...
def train(base_filters=8, dropout=0.5, weight_decay=1e-1, lr=5e-4):
    device = torch.device(config.modelconfig["device"])
    print(f"Training device: {device} | base_filters={base_filters} | dropout={dropout} | weight_decay={weight_decay}")
    ...
    input_channels = train_set[0][1].shape[0]  # condition channels (11)
    index_dim = train_set[0][3].shape[0]       # PCs + Seasonality dimension (7)
    model = UNetLiteFiLM(
    ...
    run_label = f"v4_small_bf{base_filters}_dr{dropout}_wd{weight_decay}"
    log_dir = os.path.join(config.modelconfig["log_path"] + "_" + run_label)
    ...
    for e in range(config.modelconfig["epoch"]):
        ...
        if avg_test_acc > best_test_acc:
            ...
            save_path = os.path.join(config.modelconfig["save_weight_path"], f"best_model_v4_bf{base_filters}.pt")
            torch.save(model.state_dict(), save_path)
            
            # Save as best_model_v2.pt if it beats target
    ...
    writer.close()
    print(f"\nTraining finished! Best Test ACC: {best_test_acc:.3f}")
...
if __name__ == "__main__":
    ...
    train(base_filters=args.base_filters, dropout=args.dropout, weight_decay=args.weight_decay, lr=args.lr)
