...
def prepare_data():
    """
    Load target, condition, and SST PCs. Align and return TensorDatasets.
    """
    ...
    assert len(aligned_target_dates) == cond_t.shape[0], "Mismatch in aligned dates length!"
...
def train():
    ...
    # Custom log directory for lead-1
    log_dir = os.path.join(config.modelconfig["log_path"] + "_lead1")
    ...
    for e in range(config.modelconfig["epoch"]):
        ...
        if avg_test_acc > best_test_acc:
            ...
            save_path = os.path.join(config.modelconfig["save_weight_path"], "best_model_lead1.pt")
    ...
    writer.close()
    print(f"\nTraining finished! Best Test ACC for Lead-1: {best_test_acc:.3f}")
