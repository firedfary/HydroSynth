...
def acc_hybrid_loss(pred, target, mask, mse_weight=0.01):
    """
...
def train():
    ...
    log_dir = os.path.join(config.modelconfig["log_path"] + "_v2_low_mse")
    ...
    for e in range(config.modelconfig["epoch"]):
        ...
        for x_0, cond, mask, pcs in train_loader:
            ...
            # Use smaller mse_weight=0.01 to prioritize ACC optimization
            loss = acc_hybrid_loss(out, target_clean, ~mask, mse_weight=0.01)
            loss.backward()
    ...
    writer.close()
    print(f"\nTraining finished! Best Test ACC for V2 Low MSE: {best_test_acc:.3f}")
