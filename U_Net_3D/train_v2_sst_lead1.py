...
def compute_pcs_from_sst(sst_path, n_pcs=5, window=1, step=1):
    """
    Read SST, compute EOF PCs using only lead 1 (index 1).
    """
    sst = np.load(sst_path)
    if sst.ndim == 4:
        # Select lead 1 (forecast for month t)
        sst = sst[:, 1]
        print("Selected lead-1 month-dim:", sst.shape)
...
def train():
    ...
    log_dir = os.path.join(config.modelconfig["log_path"] + "_v2_sst_lead1")
    ...
    writer.close()
    print(f"\nTraining finished! Best Test ACC for V2 SST Lead-1: {best_test_acc:.3f}")
