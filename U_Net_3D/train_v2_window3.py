...
def compute_pcs_from_sst(sst_path, n_pcs=5, window=3, step=1):
    """
    Read SST, compute EOF PCs and window them.
    """
...
def prepare_data(window=3):
    ...
    # Compute windowed PCs
    pcs, _ = compute_pcs_from_sst(sst_path, n_pcs=n_pcs, window=window, step=step)  # [364, n_pcs*window]
    ...
    # Since window=3, the first valid issue date we can use for PCs is the 3rd valid issue date
    # Valid issue dates start at 1993-12-01 (target 1994-01-01)
    # The first window starts at index 0 (1993-12-01, 1994-01-01, 1994-02-01 etc.)
    # Let's align carefully: the PCs array has shape [T_sst - window + 1, n_pcs*window]
    # The SST file has 366 months (corresponding to the 366 valid issue months)
    # So pcs[i] corresponds to issue months from valid_issue_dates[i] to valid_issue_dates[i+window-1]
    # The forecast issue month is target_date - 1 month.
    # We want to use the SST PCs ending at the issue month.
    # That means for target_date t, the issue month is t-1. The window of SST PCs ends at t-1.
    # So the window index in pcs is: issue_month_idx - window + 1
    ...
    for target_date in all_dates:
        ...
        # Get index of the issue month in valid_issue_dates
        issue_idx = date_to_idx[issue_date]
        
        # We need a window ending at issue_idx.
        # The window starts at issue_idx - window + 1.
        # Since PCs are pre-windowed, pcs[i] corresponds to window starting at index i.
        # So we select pcs[issue_idx - window + 1]
        pc_idx = issue_idx - window + 1
        if pc_idx < 0:
            continue  # Not enough history
            
        aligned_target_dates.append(target_date)
        pcs_aligned_list.append(pcs[pc_idx])
        
    # Slice cond_t and target_t to match the subset of aligned target dates
    # Originally cond_t has shape [363, 21, 120, 140].
    # But some early months might be skipped because of windowing.
    # Let's find the matching indices in the original aligned list.
    ...
    for target_date in all_dates:
        ...
        if issue_date < pd.to_datetime('1994-01-01'):
            continue
        original_target_dates.append(target_date)
        
    keep_indices = [original_target_dates.index(d) for d in aligned_target_dates]
    
    cond_t = cond_t[keep_indices]
    target_t = target_t[keep_indices]
    mask_t = mask_t[keep_indices]
...
def train():
    ...
    window = 3
    train_set, test_set = prepare_data(window=window)
    ...
    index_dim = train_set[0][3].shape[0]       # PCs dimension (n_pcs * window = 15)
    model = UNetLiteFiLM(
    ...
    log_dir = os.path.join(config.modelconfig["log_path"] + "_v2_window3")
    ...
    writer.close()
    print(f"\nTraining finished! Best Test ACC for V2 with Window=3: {best_test_acc:.3f}")
