...
def compute_pcs_from_sst(sst_path, n_pcs=3, window=3, step=1):
    """
...
def prepare_data(window=3, n_pcs=3):
...
def train():
    ...
    window = 3
    n_pcs = 3
    train_set, test_set = prepare_data(window=window, n_pcs=n_pcs)
    ...
    index_dim = train_set[0][3].shape[0]       # PCs dimension (n_pcs * window = 9)
    model = UNetLiteFiLM(
    ...
    log_dir = os.path.join(config.modelconfig["log_path"] + "_v2_pcs3_window3")
    ...
    writer.close()
    print(f"\nTraining finished! Best Test ACC for V2 with PCs=3, Window=3: {best_test_acc:.3f}")
